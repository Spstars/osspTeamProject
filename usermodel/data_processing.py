...
class Dataset(object):
    ...
    def __init__(self, config):
        ...
        data_behavior = np.load(self.data_folder + 'data_behavior.npy', allow_pickle=True)
        item_feature = np.load(self.data_folder + 'new_features.npy', allow_pickle=True, mmap_mode='r')

        self.size_item = item_feature.shape[0]
        ...
        feature = [[] for x in xrange(self.size_user)]
        self.feature_click = [[] for x in xrange(self.size_user)]
        np.save(self.data_folder + 'feature', feature)
        self.feature = []
        ...
        for user in xrange(self.size_user):
            ...
            feat = np.load(config['data_folder'] + 'feature.npy', allow_pickle=True, mmap_mode='r+')[user]
            feat = np.zeros([self.data_news_cnt[user], self.f_dim])
            np.save(config['data_folder'] + 'feat_arr', feat)
            feat_arr = np.load(config['data_folder'] + 'feat_arr.npy', allow_pickle=True, mmap_mode='r+')

            for id_ in news_dict.keys():
                feat_arr[news_dict[id_]] = item_feature[id_]
    ...
    def data_process_for_placeholder(self,
                                     user_set):  # 매트릭스 연산에 사용되는 부분.
        ...
        for u in user_set:
            ...
            for kk in xrange(min(self.band_size - 1, self.data_time[u] - 1)):
                t_indice += map(lambda x: [x + kk + 1 + sec_cnt_x, x + sec_cnt_x],
                                np.arange(self.data_time[u] - (kk + 1)))
    ...
    def prepare_validation_data(self, num_sets, valid_user):
        ...
        for ii in xrange(num_sets):
            ...
            disp_2d_split_sec_v[ii], news_cnt_short_v[ii], click_sub_index_2d_v[ii], feature_clicked_v[ii] = out[
                                                                                                                 'click_2d_x'], \
                                                                                                             out[
                                                                                                                 'disp_2d_x'], \
                                                                                                             out[
