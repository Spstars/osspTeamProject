...
def preprocess(config):
    ...
    data.drop_duplicates(subset=['user_new_index', 'timestamp', 'movie_new_index', 'is_click'], inplace=True)
    data.sort_values(by='is_click', inplace=True)
    data.drop_duplicates(keep='last', subset=['user_new_index', 'timestamp', 'movie_new_index'], inplace=True)

    sizes = data.nunique()
    size_user = sizes['user_new_index']
    size_movie = sizes['movie_new_index']
    data_user = data.groupby(by='user_new_index') # userid별로 grouping
    data_behavior = [[] for _ in xrange(
        size_user)]  # xrange 사용 이유: range는 list type으로, range가 커질수록 메모리 사용량이 커지지만, xrange는 xrange type으로 range가 커져도 메모리 사용량이 일정함
    ...
    for user in xrange(size_user):
        ...
        data_u_time = data_u.groupby(by='timestamp') # user가 본 영화들을 timestamp끼리 묶음
        time_set = np.array(list(set(data_u['timestamp'])))
    ...
    np.save(config['data_folder'] + "data_behavior", data_behavior)

    del data_behavior
    print("save data_behavior: succeed")
    new_features = np.eye(size_movie) # 영화 아이디 원-핫 인코딩 한 것
    #     genre = np.array(data.groupby('movieId').max().iloc[:,7:])들
    #     feature_mat = np.append(new_features, genre, axis=1)
    np.save(config['data_folder'] + "new_features", new_features)
