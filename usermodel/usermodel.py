...
class UserModelPW(nn.Module):
    ...
    def __init__(self, f_dim, config):
        super(UserModelPW, self).__init__()
        self.f_dim = f_dim
        # self.placeholder = {}
        self.hidden_dims = config['dims']
        self.lr = config['learning_rate']
        self.pw_dim = config['pw_dim']
        self.band_size = config['pw_band_size']
        self.mlp_model = self.mlp(self.f_dim * self.pw_dim + self.f_dim, config['dims'], 1, 1e-3, act_last=False)

    def mlp(self, x_shape, hidden_dims, output_dim, sd, act_last=False):
        hidden_dims = tuple(map(int, hidden_dims.split("-"))) # (64, 64)
    ...
    def forward(self, inputs, is_train=False, index=None):
        ...
        if is_train:
            disp_current_feature = np.array(inputs['disp_current_feature_x'])
            disp_current_feature = torch.tensor(disp_current_feature)
            ...
            disp_current_feature = np.array(inputs['feature_v'][index])
            disp_current_feature = torch.tensor(disp_current_feature)
        ...
        denseshape = [click_indices.shape[0], item_size]  # this wont work

        click_history = [[] for _ in xrange(self.pw_dim)]

        for ii in xrange(self.pw_dim):
            position_weight = torch.ones(size=[self.band_size]).to(dtype=torch.float64) * 0.0001 # weight matrix
            # print (position_weight,cumsum_tril_value_indices)

            cumsum_tril_value = position_weight[
                cumsum_tril_value_indices]  # tf.gather(position_weight, self.placeholder['cumsum_tril_value_indices'])
        ...
        concat_disp_features = torch.reshape(torch.cat([disp_history_feature, disp_current_feature], axis=1),
                                             [-1, self.f_dim * self.pw_dim + self.f_dim]) # feature 합쳐

        # (5) compute utility
        # print ("the in pu t shape s ",concat_disp_features.shape)

        u_disp = self.mlp_model(concat_disp_features.float()) # mlp 넣고 리워드 산출(input, 1)
        # net.apply(init_weights,sdv)
        # (5)
        exp_u_disp = torch.exp(u_disp) # 나온 reward에 exponential 씌워

        sum_exp_disp_ubar_ut = segment_sum(exp_u_disp, disp_2d_split_sec_ind) # exp 취한 리워드를 disp_2d_split_sec_ind 아이디에 맞게 넣음

        sum_click_u_bar_ut = u_disp[click_2d_subindex] # 기존 리워드값을 click_2d_subindex
        ...
        # loss_sum = torch.sum(torch.abs(- sum_click_u_bar_ut + torch.log(sum_exp_disp_ubar_ut + 1)))
        loss_sum = torch.sum(- sum_click_u_bar_ut + torch.log(sum_exp_disp_ubar_ut))
...
def segment_sum(data, segment_ids):
    ...
    num_segments = len(torch.unique(segment_ids)) # segment id 중복 없는 개수    return unsorted_segment_sum(data, segment_ids, num_segments)
...
def unsorted_segment_sum(data, segment_ids, num_segments):
    ...
    if len(segment_ids.shape) == 1:
        ...
        segment_ids = segment_ids.repeat_interleave(s).view(segment_ids.shape[0], *data.shape[1:]) # 0부터 num_segments 개수만큼

    assert data.shape == segment_ids.shape, "data.shape and segment_ids.shape should be equal"

    shape = [num_segments] + list(data.shape[1:])
    print(segment_ids)
    tensor = torch.zeros(*shape).scatter_add(0, segment_ids, data.float()) # reward를 segment id에 맞게 넣어
